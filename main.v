...
module TOP;
  ...
  wire mwr_arm,mem_req;  //not relevant
...
//wishbone stuff
	...
	wire SSPOE_B;
	wire ssp_sel_o;
//end of wishbone stuff



  wire	[31:0]	dataBus;
//   wire  [31:0] dataBus_slave;
...
// clear
  ...
  wire[31:0] mem_bus,main_bus;
  wire[31:0] ssp_databus; //for reading from ssp
  wire[31:0] pr_data_temp_full;
  wire[7:0] pr_data_temp;

  localparam[23:0] zero_pad = 24'b0;
  assign pr_data_temp_full ={zero_pad,pr_data_temp};

  assign ssp_databus = (mem_adr_o == 26'h0010001) ? pr_data_temp_full : mem_bus;
    
//   assign i_dataBus_slave = (ssp_w_o) ? i_dataBus_slave : ssp_databus;

  assign  main_bus = (ssp_sel_o == 1'b1) ? ssp_databus :  mem_bus;

  wishbone wb(.clk(clk), .clear_i(clear),.ssp_clear(ssp_clear),.i_addressBus_master(addressBus),
   			.i_dataBus_master(dataBus),.i_dataBus_slave(main_bus), .i_memoryRead(r),.i_memoryWrite(w),
			.i_mwr_arm(mwr_arm),.i_mem_req(mem_req),.SSPCLKIN(SSPCLKOUT),.SSPRXD(SSPTXD),.SSPFSSIN(SSPFSSOUT),
			.phi1(phi1),.phi2(phi2),.clear_o(clear_o),.mem_adr_o(mem_adr_o), .mem_r_o(mem_r_o),.mem_w_o(mem_w_o),
			.SSPCLKOUT(SSPCLKOUT),.SSPFSSOUT(SSPFSSOUT),.SSPTXD(SSPTXD),.SSPOE_B(SSPOE_B), .ssp_sel_o(ssp_sel_o),.pr_data_temp(pr_data_temp)
			);
	
  memory MEM( 	.dataBus(mem_bus), 
